...
export function ChangelogPage({ owner, repo }: ChangelogPageProps) {
  const data = buildSampleChangelogData()

  return (
    <main className="container mx-auto flex min-h-[calc(100vh-6rem)] max-w-5xl flex-col gap-8 px-6 py-10 lg:py-16">
      <header className="flex items-start justify-between gap-4 rounded-3xl border border-border/70 bg-card/80 px-6 py-5 shadow-lg">
        <div className="flex-1">
          ...
          <p className="mt-2 flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>·</span>
            <a
              href={`https://github.com/${owner}/${repo}/tree/${data.pullRequest.branch}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2 hover:text-foreground transition-colors"
            >
        ...
        <a
          href={`https://github.com/${owner}/${repo}/pull/${data.pullRequest.number}`}
          target="_blank"
          rel="noopener noreferrer"
...
function ChangelogSummaryCard({
  ...
  const handleShareOnX = () => {
    const changelogText = buildChangelogText()
    const shareUrl = `https://github.com/${owner}/${repo}`
    ...
    <div className="space-y-4">
      <div className="space-y-3">
        <div className="flex-1 space-y-3">
          <Kanji title="Henka - Change." className="text-xs">
            変更
          </Kanji>
...
function FeatureFilmCard() {
  return (
    <div className="space-y-4">
      <div className="space-y-3">
        <Kanji title="Film." className="text-xs">
          映画
        </Kanji>
...
function StoriesCarousel({
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-3">
        <Kanji title="Sutōrī - Stories." className="text-xs">
          ストーリー
        </Kanji>
          ...
          <div>
            {(() => {
              ...
              const lineAnchor = lineRange
                ...
                : ''
              const githubUrl = `https://github.com/${owner}/${repo}/blob/${branch}/${filePath}${lineAnchor}`
              return (
                <ShikiCodeBlock
