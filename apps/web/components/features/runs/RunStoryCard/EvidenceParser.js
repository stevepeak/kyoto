...
function parseEvidenceToGitHubLink(evidence, orgName, repoName, commitSha) {
    ...
    var match = evidence.match(/^(.+):(\d+)-(\d+)$/);
    if (!match) {
        return { text: evidence, url: null };
    }
    var filePath = match[1], startLine = match[2], endLine = match[3];
    if (!commitSha) {
        return { text: evidence, url: null };
    }
    var url = "https://github.com/".concat(orgName, "/").concat(repoName, "/blob/").concat(commitSha, "/").concat(filePath, "#L").concat(startLine, "-L").concat(endLine);
    return { text: evidence, url: url };
}
